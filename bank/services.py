...
def _account_from_row(row) -> Account:
    return Account(id=row['id'], name=row['name'], balance=_to_rupees(row['balance']), created_at=row['created_at'])
...
class BankService:
    """All public methods take/return plain float rupee amounts. Internally,
    money is stored and moved as integer paise so repeated transactions can't
    ...
    def create_account(self, name: str, initial_deposit: float = 0.0) -> Account:
        ...
        paise = _to_paise(initial_deposit)
        conn = get_conn(self.db_path)
        try:
            with conn:
                cur = conn.execute('INSERT INTO accounts (name, balance) VALUES (?, ?)', (name, paise))
                account_id = cur.lastrowid
                if paise > 0:
                    conn.execute(
                        'INSERT INTO transactions (account_id, type, amount, note) VALUES (?, ?, ?, ?)',
                        (account_id, 'deposit', paise, 'initial deposit'),
                    )
    ...
    def deposit(self, account_id: int, amount: float, note: str = '') -> bool:
        ...
        try:
            with conn:
                cur = conn.execute('UPDATE accounts SET balance = balance + ? WHERE id = ?', (paise, account_id))
                if cur.rowcount == 0:
                    raise ValueError('Account not found')
                conn.execute(
                    'INSERT INTO transactions (account_id, type, amount, note) VALUES (?, ?, ?, ?)',
                    (account_id, 'deposit', paise, note),
                )
            return True
        finally:
            conn.close()
        ...
        try:
            with conn:
                ...
                cur = conn.execute(
                    ...
                    (paise, account_id, paise),
                )
                if cur.rowcount == 0:
                    raise ValueError('Insufficient funds')
                conn.execute(
                    'INSERT INTO transactions (account_id, type, amount, note) VALUES (?, ?, ?, ?)',
                    (account_id, 'withdraw', paise, note),
                )
            return True
        finally:
            conn.close()
        ...
        try:
            with conn:
                ...
                if cur.rowcount == 0:
                    ...
                    raise ValueError('Insufficient funds')
                conn.execute('UPDATE accounts SET balance = balance + ? WHERE id = ?', (paise, to_id))
                conn.execute(
                    'INSERT INTO transactions (account_id, type, amount, counterparty, note) VALUES (?, ?, ?, ?, ?)',
                    (from_id, 'transfer-out', -paise, to_id, note),
                )
                conn.execute(
                    'INSERT INTO transactions (account_id, type, amount, counterparty, note) VALUES (?, ?, ?, ?, ?)',
                    (to_id, 'transfer-in', paise, from_id, note),
                )
            return True
        finally:
            conn.close()
